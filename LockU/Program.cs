...
if (initiallyDetected)
{
    while (true)
    {
        ...
        if (!IsUsbDetected())
        {
            ...
            Thread.Sleep(TimeSpan.FromSeconds(1));
            Simulator.Keyboard.TextEntry("kwispel");
            Thread.Sleep(TimeSpan.FromSeconds(1));
            Simulator.Keyboard.KeyPress(WindowsInput.Native.VirtualKeyCode.RETURN);
            Thread.Sleep(TimeSpan.FromSeconds(1));
        }

    https://stackoverflow.com/questions/5290526/loging-to-windows-7-using-the-application-opened-on-a-locked-workstation
